...
final class FeedRepository extends Repository
{
    ...
    {
        $t_feeds = self::TABLE_NAME;
        $selectColumns = self::COLUMN_NAMES;
        $c_status = Feed::FIELD_STATUS;
        $c_nextUpdate = Feed::FIELD_NEXT_UPDATE;
        
        $q = "SELECT (`${selectColumns}`) FROM `${t_feeds}` " . 
            "WHERE `${c_status}` <> ? AND `${c_nextUpdate}` <= ? " . 
            "ORDER BY `${c_nextUpdate}`";
        
        return $this->db->queryObjects($q, Feed::class, 'ss', 
            FeedStatus::BANNED->value, Util::dateTimeToString($nextUpdateBefore));
    }
    ...
    public function update(DateTime $lastUpdate, DateTime $nextUpdate, string $lastEntry, int $ID) : int
    {
        ...
        $q = "UPDATE `${t_feeds}` " . 
            "SET `${c_lastUpdate}` = ?, `${c_nextUpdate}` = ?, `${c_lastEntry}` = ? " .
            "WHERE `{$c_ID}` = ? LIMIT 1";

        return $this->db->execute($q, 'sssi', 
            Util::dateTimeToString($lastUpdate), Util::dateTimeToString($nextUpdate), $lastEntry, $ID);
    }
    ...
    public function save(Feed $feed) : int
    {
        ...
        if ($feed->getID() < 0) {
            $affectedRows = $this->db->execute($q, 'sissssss', ...$values);
            if ($affectedRows > 0) {
                $feed->setID($this->db->getLastInsertID());
            }
        } else {
            $affectedRows = $this->db->execute($q, 'isissssss', ...$values);
        }
