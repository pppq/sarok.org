...
class TextProcessor {
    ...
    private const ALLOWED_TAGS = '<a><h1><h2><h3><center><i><b><span><div><ul><li><small><big><sup><sub><strike><u><input>' .
                                 '<select><button><pre><hr><blockquote><code><img><object><param><embed><nowrap><select>' .
                                 '<option><form><iframe><br>';
    
    private const WRONG_WINDOW = 'Tegnap megismerkedtem egy fiúval és azóta csak rá gondolok. Gyönyörű göndör haja van, kék ' .
                                 'szeme és vastag farka, amely alig fér be a se... Bocs, azt hiszem rossz ablakba írtam!';
    ...
    public function postFormat(string $text, string $searchKeyword = '') : string {
        ...
        if (isset($text) && strlen($text) > 0) {
             ...
             */
            if (isset($searchKeyword) && strlen($searchKeyword) > 0) {
